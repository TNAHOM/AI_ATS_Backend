...
Create a Clerk JWT template (for example `backend-jwt`) and use it consistently for backend requests.
...
2. Frontend requests backend token from Clerk template (`backend-jwt`).
...
export async function GET() {
  ...
  const token = await getToken({ template: "backend-jwt" });
...
export function useBackendApi() {
  ...
  const callBackend = async (path: string, init?: RequestInit) => {
    ...
    let token = await getToken({ template: "backend-jwt" });
    ...
    if (response.status === 401) {
      token = await getToken({ template: "backend-jwt", skipCache: true });
      if (!token) throw new Error("No auth token available");
...
- [ ] JWT template exists (e.g., `backend-jwt`)
- [ ] Template includes `aud` and `email` claims
- [ ] Frontend uses `getToken({ template: "backend-jwt" })`
